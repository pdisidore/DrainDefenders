...
void loop() {
  ...
  if (timeInMilli >= 1000) {
    ...
    //double turb = 730 - analogRead(24);
    //Serial.println(turb);

    lastMsg = now;
    if (i < 32) {
      modem.getBattVoltage(&vbat);
      VbatBuffer[i] = vbat;
      VbatFor32Readings += vbat;
      i++;
    } else {

      uint16_t battLevel = VbatFor32Readings >> 5;
      Serial.print(F("VBat = "));
      ...
      sprintf(URL, "dweet.io/dweet/for/%s?turb=%s&batt=%i", imei, turbSendBuff, battLevel);

      if (!modem.postData("GET", URL)) Serial.println(F("Failed to complete HTTP GET..."));
